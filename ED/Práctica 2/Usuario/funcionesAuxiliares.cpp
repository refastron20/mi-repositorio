...
void ed::testPolinomio()
{
  ...
  std::cout << std::endl << std::endl;
	...
	if (p1.esNulo())
		std::cout << "P1 es un polinomio nulo" << std::endl;
	else
		std::cout << "P1 no es un polinomio nulo" << std::endl;
  ...
  std::cin.ignore();
	...
	// Operador de división y asignación
	std::cout<< "Operador p3 /= m "<< std::endl;
	if (p3.getGrado() >= m.getGrado() and (m!=0))
	{
		p3 /= m;
	}
	else
    ...
    std::cout << "-No se puede dividir p3 entre m, porque m es nulo" << std::endl;
	}
